...
# drawing on a copy sized to the display pixels (avoids click drift)
def make_display_image(base_img: Image.Image, disp_w: int, state) -> Tuple[Image.Image, float]:
    ...
    elif len(state.prox) == 1:
        d.ellipse([state.prox[0][0]*scale-3, state.prox[0][1]*scale-3,
                   state.prox[0][0]*scale+3, state.prox[0][1]*scale+3],
                  fill=(66,133,244,255))
    ...
    elif len(state.dist) == 1:
        d.ellipse([state.dist[0][0]*scale-3, state.dist[0][1]*scale-3,
                   state.dist[0][0]*scale+3, state.dist[0][1]*scale+3],
                  fill=(221,0,221,255))
...
# ------------------------- UI & state -------------------------
ss = st.session_state
defaults = dict(poly=[], cora=None, hinge=None, prox=[], dist=[], ruler=[],
...
ss.segment = st.sidebar.radio("Move segment", ["distal","proximal"], index=(0 if ss.segment=="distal" else 1), horizontal=True)
...
# Build the exact display image (so click coords match 1:1)
disp_img, scale = make_display_image(img, ss.dispw, ss)

# Show clickable image (no container resizing; width is exact)
res = streamlit_image_coordinates(disp_img, width=disp_img.width, key="clicks")
...
if res and "x" in res and "y" in res:
    ...
    if tool == "Polygon":
        ...
        # persistent: keep existing unless you intentionally start a new one
        if len(ss.prox) >= 2:                   # third click starts a new line
            ss.prox = [pt]
        else:
            ss.prox.append(pt)

    elif tool == "Dist line":
        if len(ss.dist) >= 2:                   # third click starts a new line
            ss.dist = [pt]
        else:
            ss.dist.append(pt)
...
if len(ss.poly) >= 3 and center is not None:
    ...
    # redraw lines with proper following
    ...
    st.image(disp_out, use_container_width=False)
    ...
    st.info("Draw polygon (≥3) and set HINGE/CORA. Clicks should land 1:1 with your cursor. Lines persist across tools. To start a new line, click a third time on that tool.")
