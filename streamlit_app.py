...
# reset click stream when tool switches
...
# ---------- Canvas utilities (make sure background always shows) ----------
def _png_data_url(pil_img: Image.Image) -> str:
    if pil_img.mode != "RGB":
        pil_img = pil_img.convert("RGB")
    bio = BytesIO(); pil_img.save(bio, format="PNG")
...
def _canvas_show(bg_pil: Image.Image, **kwargs):
    """
    Try (A) both background_image + initial_drawing,
        (B) background_image only,
        (C) initial_drawing only.
    This avoids the 'blank canvas' quirk across different builds.
    """
    W, H = bg_pil.size
    ...
    try:
        return _call(
            initial_drawing=scene, background_image=bg_pil,
            width=W, height=H, background_color=None, **kwargs
        )
    except Exception:
        pass
    # B) background only
    try:
        return _call(
            background_image=bg_pil, width=W, height=H,
    ...
    # C) initial only
    return _call(
        initial_drawing=scene, width=W, height=H,
        background_color=None, **kwargs
    )

def _close_if_near_first(pts, scale, px_thresh=25):
    """If last vertex is within px_thresh (display px) of first, snap/close."""
    if len(pts) < 3:
        return pts
    x0, y0 = pts[0]
    xn, yn = pts[-1]
    ...
    if result.json_data:
        ...
        for obj in reversed(objs):
            ...
            if t == "polygon":
                ...
                pts = _close_if_near_first(pts, scale, px_thresh=25)
                pts = _force_closed(pts)
                if len(pts) >= 3:
                    return pts
...
if len(ss.poly) >= 3 and center is not None:
    ...
    # redraw lines according to moving segment
