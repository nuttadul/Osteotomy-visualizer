...
def make_display_image(base_img: Image.Image, disp_w: int, state) -> tuple[Image.Image, float]:
    ...
    if len(state.prox) == 2:
        ...
        x, y = state.prox[0]
        d.ellipse([x*scale - 3, y*scale - 3, x*scale + 3, y*scale + 3], fill=(66, 133, 244, 255))
    ...
    if len(state.dist) == 2:
        ...
        x, y = state.dist[0]
        d.ellipse([x*scale - 3, y*scale - 3, x*scale + 3, y*scale + 3], fill=(221, 0, 221, 255))
...
tool = st.sidebar.radio(
    "Tool",
    ["Polygon", "CORA", "HINGE", "Prox line", "Dist line", "Ruler"],
    index=0
)

# bump the click key when tool changes → prevents “ghost clicks”
...
# explicit “start a new line” buttons so lines persist until YOU choose
cc1, cc2 = st.sidebar.columns(2)
if cc1.button("New Prox line"): ss.prox = []
if cc2.button("New Dist line"): ss.dist = []
...
if len(ss.poly) >= 3 and center is not None:
    ...
    # redraw lines (distal follows distal motion; proximal follows proximal motion)
    ...
    # exact-size display; don't pass use_container_width to avoid version issues
    ...
    st.info("Draw polygon (≥3) and set HINGE/CORA. Lines persist; click “New Prox line/Dist line” when you want to start a new one.")
